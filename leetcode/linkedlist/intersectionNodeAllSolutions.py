...
思路：for each node a in list A, traverse the entire list B and check if any node
in list B coincides with a
方法：内外两个while循环（for循环需要计算list长度），不断迭代，遇到相同地址的则返回node，
全不相同则返回None
...
方法：A中每一个node都不一样，故可以用list/hash table(dic/set)实现存储结构，
